...
`src/_includes/diagrams/` 下每个工作一张内联 SVG，按 VoiceMem 项目页的思路：一张图就是这项工作的核心机制，不是流程图。

| 图 | 隐喻 | 画的是什么 |
|---|---|---|
| `dfm.njk` | 年轮螺旋 | 六项发现能力落在六条射线上，螺旋每圈再经过一次（递归）、每圈更大（沉淀）、开口不闭合（范式而非成品）；琥珀点在"根据外部证据验证和更新判断" |
| `sciencebuddy.njk` | 双纽线 | 左瓣科学家、右瓣智能体，唯一的交点就是反馈；六个动词是左瓣上的刻度；下方是研究轨迹基线。形状复用了公司标志，需创始人确认 |
| `scienceide.njk` | 晶格与晶胞 | 等轴测格墙 = 基础设施（行 = 五种任务形态，列 = 代码库，格 = 一个环境）；放大的晶胞三面（代码·领域判断 / 任务目标 / 可验证反馈）交于一点 |
| `jepa.njk` | 光锥 | 过去锥里六个领域的观测汇入潜在状态 z，未来锥里三条干预世界线，一条预测可行（琥珀），两条被筛掉；末段按正交因子分解 |
| `program.njk` | 迈克尔逊干涉仪 | 真实问题入射，科学家臂与 PhAI 臂缺一不可，两束返回才有条纹 = 可检验的判断；分束器（琥珀）就是申请入口 |

图内所有标签逐字取自 PR Brief 对应段落；隐喻名是设计命名。每张图只有一个琥珀元素。动效全部是 CSS（进入视口时描线，`offset-path` 上的光点，脉冲），鼠标悬停高亮对应分组，`prefers-reduced-motion` 下显示画完的静图。
