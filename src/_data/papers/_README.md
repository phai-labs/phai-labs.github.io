...
| `bibtitle` | Optional. The title with braces around the terms whose capitalisation must survive a BibTeX style that lower-cases titles, e.g. `{Discovery Foundation Models}: Toward Open-Ended {Discovery Intelligence}`. Qwen's own `@techreport` brace-protects `{Qwen3.8-Next}` the same way. Omit it and the plain `title` is used. |
